...
I would drop PC5 and PC6 and PC7, because they account for a relative small proportion of variance and PC1-4 account for nearly 80% variance.
...
D4 <- cbind(D4, as.data.frame(D1$mean_correct))
...
```{r}
df1 <- read.csv("TCprograms.csv", header = TRUE, sep =  " ")
df1 <- df1[,-1]
...
m <- sweep(loadings2, 2, colSums(loadings2), "/")
biplot(pca2)
#Interpretation: According to PC1, we can see the higher correlation in "change leadership""Economic and education"and "Educational policy". In PC2, higher correlation in Kinesiology, Clinical Psychology and Physiology. PC3 describes higher correlation among Learning Analytics, Cognitive Science and Mathematics.
